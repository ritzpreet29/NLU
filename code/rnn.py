...
class RNN(object):
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			...
			s_out = np.subtract(d_onehot,y[t])
			f_net = np.multiply(s[t], (np.subtract(1, s[t])))
			s_in = np.multiply(np.dot((np.transpose (self.W)), s_out), f_net)

			self.deltaW +=  np.outer(s_out, s[t])

			self.deltaV += np.outer(s_in, make_onehot(x[t], self.vocab_size))

			self.deltaU += np.outer(s_in, s[t-1])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			#make_onehot(x[t-steps], self.vocab_size)
			for T in range(steps):

				dt_onehot = make_onehot(d[t-T], self.vocab_size)
				s_out_steps = np.subtract(dt_onehot, y[t-T])
				f_net_steps = np.multiply(s[t-T], (np.subtract(1, s[t-T])))
				U_T = np.transpose(self.U)

				s_in_new = np.multiply(np.dot((np.transpose (self.W)), s_out_steps), f_net_steps)
				s_in_steps = np.multiply(np.dot(U_T, s_in_new), f_net_steps)



				self.deltaV += np.outer(s_in_steps, make_onehot(x[t-T], self.vocab_size))
				self.deltaU += np.outer(s_in_steps, s[t-T-1])
	...
	def compute_loss(self, x, d):
		...
		for t in range(len(x)):
			...
			loss += -1 * (np.dot(np.log(y[t]), d_onehot))
	...
	def compute_mean_loss(self, X, D):
		...
		for n, sentence in enumerate(X):
			total += self.compute_loss(sentence, D[n])

			mean_loss = total / sum(len(list) for list in D)
